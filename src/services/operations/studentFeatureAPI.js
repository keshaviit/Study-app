...
export async function buyCourse(token, courses, userDetails, navigate, dispatch) {
    ...
    try{
        ...
        // Verify the Razorpay order object structure
        const razorpayOrder = orderResponse.data.message;
        if (!razorpayOrder || !razorpayOrder.id || !razorpayOrder.amount || !razorpayOrder.currency) {
        ...
        if (!razorpayKey) {
            toast.error("Missing Razorpay key (set VITE_RAZORPAY_KEY in .env)")
            return
        }
        const options = {
            key: razorpayKey,
